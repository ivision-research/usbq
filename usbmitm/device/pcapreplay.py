...
class PcapReplay(USBDevice):
    ...
    def create_arg_subparser(cls, parser):
        ...
        parser.add_argument(
            ...
            default=HIGH_SPEED,
            type=int,
            help="Speed of device (default HIGH_SPEED",
        )
    ...
    def has_to_handle_pkt(self, pkt):
        ...
        return (
            pkt.is_ctrl_request()
            or pkt.is_ctrl_response()
            or (pkt.urb_type == SUBMIT and pkt.endpoint_direction == OUT)
            or (pkt.urb_type == COMPLETE and pkt.endpoint_direction == IN)
        )
    ...
    def load_identity(self, pcap, speed):
        ...
        for pkt in rdpcap(pcap):
            ...
            if pkt.is_ctrl_request():
                ...
                if (
                    urb.direction == 1
                    and urb.type == 0
                    and pkt.urb_setup.bDescriptorType in (1, 2, 3)
                ):
                ...
                if len(self.descs[desc]) < index + 1:
                    self.descs[desc].extend(
                        (index - len(self.descs[desc])) * [(None, 0)]
                    )
        ...
        for key, value in self.descs.items():
            descs[key] = [x[0] for x in self.descs[key]]
    ...
    def is_enumeration_request(self, msg):
        ...
        return (
            msg.ep.is_ctrl_0()
            and hasattr(msg.request, "bDescriptorType")
            and msg.request.bDescriptorType in self.descs
        )
    ...
    def is_enumeration_response(self, msg):
        ...
        return (
            msg.ep.is_ctrl_0()
            and hasattr(msg.response, "bDescriptorType")
            and msg.response.bDescriptorType in self.descs
        )
            ...
            if self.will_be_received_pkt(pkt):
                if self.is_enumeration_request(pkt):
                    if not self.is_handle(pkt.urb_setup):
                        ...
                        for i in range(pkt_num, len(pkts)):
                                ...
                                if pkt.endpoint_direction == IN:
                                    for j in range(i + 1, len(pkts)):
                                        if self.is_enumeration_response(pkts[j]):
                                            ...
                                            ep = USBEp(
                                                epnum=npkt.endpoint_number,
                                                eptype=pcaptype_to_eptype[
                                                    npkt.urb_transfert
                                                ],
                                                epdir=PROTO_IN
                                                if npkt.endpoint_direction == IN
                                                else PROTO_OUT,
                                            )
                                            if npkt.urb_status == 0:
                                                self.send_pcap_pkt(
                                                    ep,
                                                    req=pkt.urb_setup,
                                                    resp=npkt.descriptor,
                                                    data=npkt.data,
                                                )
                                            else:
                                                self.send_ack(ep, -32)
                                                print(
                                                    "Packet error so not sending data"
                                                )
                                            break
                                        else:
                                            print("Not resp %u" % (j,))
                                        ...
                                        print(
                                            "Do not know what is the next pkt %r"
                                            % (pkts[i],)
                                        )
                                        self.disconnect()
                                        sys.exit(0)
                ...
                if self.is_enumeration_response(pkt):
                    ...
                    print("REPLAY")
                    ep = USBEp(
                        epnum=pkt.endpoint_number,
                        eptype=pcaptype_to_eptype[pkt.urb_transfert],
                        epdir=PROTO_IN if pkt.endpoint_direction == IN else PROTO_OUT,
                    )
