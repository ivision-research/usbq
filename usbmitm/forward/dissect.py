...
class Dissector(Forwarder):
    ...
    def create_arg_subparser(cls,parser):
        parser.add_argument("--no-log-device",action="store_true",help="Print message get from Device")
        parser.add_argument("--no-log-host",action="store_true",help="Print responses get from Host")
        parser.add_argument("--debug-mitm",action="store_true",help="Print all USBMitm packet")
        parser.add_argument("--dissect",action="store_true",help="Dissect USB Packets")
        parser.add_argument("--full",action="store_true",help="Dissect Full USB Packets")
    ...
    def __init__(self,args):
        ...
        func = lambda d,ep: "%s%s%u: %s" % (abv_type[ep[1]],abv_dir[ep[2]],ep[0],self._output(d)) if ep is not None else "MNGT: %s" % (self._output(d),)

        if self.debug_mitm:
            if args.dissect:
                self.fmt = (lambda d,ep: func(USBMessageDevice(d),ep),
                            lambda d,ep: func(USBMessageHost(d),ep))
            else:
                self.fmt = (lambda d,ep: func(lhex(d),ep),
                            lambda d,ep: func(lhex(d),ep))
        else:
            if args.dissect:
                self.fmt = (lambda d,ep: func(USBMessageResponse(extract_usb_part(d)).get_usb_payload(),ep) if is_usb_msg(d) else None,
                            lambda d,ep: func(USBMessageRequest(extract_usb_part(d)).get_usb_payload(),ep) if is_usb_msg(d) else None)
            else:
                self.fmt = (lambda d,ep: func(lhex(extract_usb_part(d)),ep) if is_usb_msg(d) else None,
                            lambda d,ep: func(lhex(extract_usb_part(d)),ep) if is_usb_msg(d) else None)
