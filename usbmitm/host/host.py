...
class USBHost(Receiver):
    ...
    def __init__(self,args,descriptors_needed=[DEVICE_DESCRIPTOR,CONFIGURATION_DESCRIPTOR,STRING_DESCRIPTOR]):
    ...
    def recv_desc(self,rep):
        ...
        if hasattr(self,f):
            ...
            try:
                ...
                return
            next_req = GetDescriptor(bDescriptorType=self.descriptors_needed[new_index],wLength=255)
    ...
    def get_next_req_configuration(self,rep):
        ...
        if len(rep) != rep.wTotalLength:
            req = GetDescriptor(bDescriptorType=CONFIGURATION_DESCRIPTOR,wLength=rep.wTotalLength)
        else:
