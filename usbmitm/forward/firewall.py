...
class Firewall(Forwarder):
    ...
    def create_arg_subparser(cls, parser):
        ...
        parser.add_argument(
            ...
            metavar="CLASS_ID",
            type=get_class,
            help="Allow class (integer or string: [%s])"
            % (",".join(list(classid.keys()))),
        )
        parser.add_argument(
            "--filtering", metavar="FILTERING_RULE", help="Filtering rule"
        )
    ...
    def hookDevice(self, data):
        ...
        try:
            ...
            # During two requests
                ...
                if msg.ep.is_ctrl_0():
                    if (
                        type(msg.request) is GetDescriptor
                        and msg.request.bDescriptorType == DEVICE_DESCRIPTOR
                        and self.device_desc != str(msg.response)
                    ):
                        msg.request.show()
                        error(
                            "Device descriptor modifications... resetting and blocking"
                        )
                        self.reset()
                        self.reset_to_host()
                    if type(msg.response) is ConfigurationDescriptor:
                        msg.response.descriptors = self.clean_descriptors(
                            msg.response.descriptors
                        )
                        sdata = str(msg.response)
                        if len(sdata) > 9 and self.conf_desc != sdata:
                            error(
                                "Configuration descriptor modifications... resetting and blocking"
                            )
                            self.reset()
                            self.reset_to_host()
    ...
    def hook_management(self, msg):
        ...
        if msg.management_type == 0:
            ...
            msg.management_content.configuration.descriptors = self.clean_descriptors(
                msg.management_content.configuration.descriptors
            )
