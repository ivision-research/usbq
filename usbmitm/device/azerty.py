...
invkeyMap = {
    "none": {v: k for k, v in list(keyMap["none"].items())},
    "shift": {v: k for k, v in list(keyMap["shift"].items())},
    "altgr": {v: k for k, v in list(keyMap["altgr"].items())},
}

_modifier = {"none": 0, "shift": KEY_SHIFT_LEFT, "altgr": KEY_ALTGR}


def get_scan_code(char):
    for mod in list(keyMap.keys()):
        if char in keyMap[mod]:
            return "%c\x00%c\x00\x00\x00\x00\x00" % (
                chr(_modifier[mod]),
                chr(keyMap[mod][char]),
            )
...
def get_char(scancode):
    ...
    try:
        if mod & KEY_SHIFT != 0:
            ...
            return "%s%s" % (
                get_modifier(mod),
                invkeyMap["none"].get(val, "ukn%r" % (val,)),
            )
    except KeyError:
        return "%s%s" % (
            get_modifier(mod),
