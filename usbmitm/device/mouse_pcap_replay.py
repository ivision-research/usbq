...
report_descriptor = "05010902a1010901a100050919012903150025019503750181029505810305010930093109381581257f750895038106c0c0".decode("hex")
hid_descriptor = HIDDescriptor(descriptors=[HIDReportDescriptor(wDescriptorLength=len(report_descriptor))])
MouseInterface = Interface(descriptors=[Endpoint(1,INT,IN,4,ls2hs_interval(10)),hid_descriptor],cls=HID,subcls=1,proto=HID_PROTOCOL_MOUSE)

class Mouse(HIDDevice):
    def __init__(self,args):
        ident = DeviceIdentity.from_interface(MouseInterface,speed=LOW_SPEED)
        ident.set_strings(["\x09\x04","USBiquitous emulated mouse".encode('utf-16le'),"USBIQUITOUS".encode('utf-16le')])
...
class MousePCAP(Mouse):
    ...
    def match(self,pkt):
        ...
        return pkt.urb_type == COMPLETE and pkt.urb_transfert == PCAP_INT and pkt.endpoint_direction == IN
