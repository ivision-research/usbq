...
class USBMutation(Dissector):
    ...
    def create_arg_subparser(cls, parser):
        parser.add_argument(
            ...
            "-H",
            action="store_true",
            help="Apply mutation on data sent by host",
        )
        parser.add_argument(
            "--mutation-device",
            "-D",
            action="store_true",
            help="Apply mutation on data sent by device",
        )
        parser.add_argument(
            "--timer",
            "-t",
            ...
            type=int,
            default=10,
            help="Number of seconds before reloading communication",
        )
        parser.add_argument(
            "--seed",
            metavar="SEED",
            type=int,
            help="Set seed for random values to be reproductible",
        )
        parser.add_argument(
            "--bytes",
            "-B",
            action="store_true",
            help="If set then corrupt byte will be set instead of corrupt bits",
        )
        parser.add_argument(
            "--percentage-pkt",
            "-P",
            ...
            default=0.01,
            type=float,
            help="Percentage of packet modified",
        )
        parser.add_argument(
            "--number",
            "-n",
            ...
            default=None,
            type=int,
            help="Number of bits/bytes fuzzed inside a packet",
        )
        parser.add_argument(
            "--percentage",
            "-c",
            ...
            default=0.01,
            type=float,
            help="Percentage of bits/bytes fuzzed inside a packet (will be override by number if set)",
        )
        parser.add_argument(
            "endpoints",
            metavar="ENDPOINTS",
            nargs="*",
            help="Endpoints to fuzz: ctr1in,bul0out,int1out... (empty for all) ",
        )
    ...
    def shall_modify(self, msg):
        ...
        return (
            self.match(msg.ep) and float(random.randint(0, 100)) < args.percentage_pkt
        )
