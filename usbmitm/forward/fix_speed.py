...
class FixLowSpeed(Dissector):
    ...
    def create_arg_subparser(cls,parser):
        parser.add_argument("--no-pkt-sz",action="store_true",help="Do not fix DeviceDescriptor max packet size")
        parser.add_argument("--no-pkt-interval",action="store_true",help="Do not fix EndpointDescriptors interval")
        parser.add_argument("--no-usb-version",action="store_true",help="Do not usb version")
    ...
    def fix_pkt_interval(self,configuration_descriptor):
        ...
        for endpoint_descriptor in configuration_descriptor.descriptors:
            if isinstance(endpoint_descriptor,EndpointDescriptor):
                endpoint_descriptor.bInterval = ls2hs_interval(endpoint_descriptor.bInterval)
        return configuration_descriptor
